...
public class DateParser {
    ...
    private int getTime(int stringStart, int stringEnd, String characterCount, String isInt, int minTime, int maxTime, String OutOfRange) {
        int year;
        try {
            String yearString = dateAndTimeString.substring(stringStart, stringEnd);
            year = Integer.parseInt(yearString);
        } catch (StringIndexOutOfBoundsException e) {
            throw new IllegalArgumentException(characterCount);
        } catch (NumberFormatException e) {
            throw new IllegalArgumentException(isInt);
        }
